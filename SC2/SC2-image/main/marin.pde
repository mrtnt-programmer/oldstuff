...
class Marin{
  ...
  Marin(float Xdonne,float Ydonne,int team,int combatdonne,String nomuniterdonne){
    ...
    if (istheretarget){
      ...
      if(myteam == 1){//si je suis vert la base je fonce dessu
        if (mybody.x >= (gamewidth/16)*10){//11 mais je metre 10 pour etre plus naturelle
          if (findunit("nexus",2)){
            vitesse.set(marins.get(findunit).mybody.x - mybody.x,marins.get(findunit).mybody.y - mybody.y);
            vitesse.normalize();
            mybody.add(vitesse);
          }else{
            avancer();
  ...
  void afficher(){
    ...
    circle(K*mybody.x+locationX,K*mybody.y+locationY,K*mysize);//radius = 5pix ou width/384//////////////////////////////////////////////////////////////////////////////////////////////////////////////
    
   if (myteam == 2){
   image(marinstill2,K*mybody.x+locationX-mysize/2,K*mybody.y+locationY-mysize/2);
  }else{
   image(marinstill,K*mybody.x+locationX-mysize/2,K*mybody.y+locationY-mysize/2);
  }
  }
  ...
  boolean findunit(String name,int team){//donner le nom de l'uniter, donner l`equipe
    for(int i=0;i<marins.size();i++){
      if (marins.get(i).nomuniter == name &&marins.get(i).myteam == team){
        findunit = i;
        return true;
      }
    }
